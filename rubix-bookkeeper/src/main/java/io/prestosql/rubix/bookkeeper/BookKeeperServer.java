...
public class BookKeeperServer extends Configured implements Tool
{
  ...
  private void createThriftServer(Configuration conf, BookKeeper bookKeeper) {
    ...
    log.info("Starting BookKeeperServer on port " + CacheConfig.getBookKeeperServerPort(conf));
    try {
      TServerTransport serverTransport = new TServerSocket(
              new TServerSocket.ServerSocketTransportArgs().bindAddr(new InetSocketAddress(CacheConfig.getBookKeeperServerPort(conf))).backlog(Integer.MAX_VALUE));
      server = new TThreadPoolServer(new TThreadPoolServer
          .Args(serverTransport)
          .processor(processor)
          .maxWorkerThreads(CacheConfig.getServerMaxThreads(conf)));
    }
