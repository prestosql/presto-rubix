...
import static io.prestosql.rubix.spi.CommonUtilities.toBlockStartPosition;
import static io.prestosql.rubix.spi.CommonUtilities.toEndBlock;
import static io.prestosql.rubix.spi.CommonUtilities.toStartBlock;
    ...
    for (Map.Entry<String, DownloadRequestContext> entry : contextMap.entrySet()) {
      ...
      for (Range<Long> range : context.getRanges().asRanges()) {
        ...
        try {
          ...
          if(generationNumber != CacheUtil.UNKONWN_GENERATION_NUMBER && response.getGenerationNumber() != generationNumber) {
