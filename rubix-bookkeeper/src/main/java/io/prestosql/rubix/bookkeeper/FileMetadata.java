...
public class FileMetadata
{
      ...
      int generationNumber)
  {
    ...
    int bitsRequired = (int) Math.ceil((double) size / CacheConfig.getBlockSize(conf)); //numBlocks
      ...
      throws ExecutionException
  {
    ...
    if (CacheConfig.isDummyModeEnabled(conf)) {
      return CacheUtil.DUMMY_MODE_GENERATION_NUMBER;
    }
    ...
    if (!fileAccessedBloomFilter.mightContain(remotePath)) {
      ...
      int highestGenNumberOnDisk = CacheUtil.UNKONWN_GENERATION_NUMBER + 1;
      ...
      if (CacheConfig.isCleanupFilesDuringStartEnabled(conf)) {
        ...
        if (highestGenNumberOnDisk == CacheUtil.UNKONWN_GENERATION_NUMBER) {
          genNumber = 1;
        }
      ...
      genNumber = generationNumberCache.get(remotePath, () -> CacheUtil.UNKONWN_GENERATION_NUMBER) + 1;
