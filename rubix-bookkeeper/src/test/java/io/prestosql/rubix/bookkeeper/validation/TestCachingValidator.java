 ...
 */
package io.prestosql.rubix.bookkeeper.validation;

import com.codahale.metrics.MetricRegistry;
import com.google.common.collect.Lists;
import io.prestosql.rubix.bookkeeper.BookKeeper;
import io.prestosql.rubix.bookkeeper.CoordinatorBookKeeper;
...
import io.prestosql.rubix.spi.fop.Poolable;
...
public class TestCachingValidator
{
  ...
  private static final CacheStatusResponse TEST_LOCATIONS_CACHED = new CacheStatusResponse(Lists.newArrayList(new BlockLocation(Location.CACHED, TEST_REMOTE_LOCATION)), CacheUtil.UNKONWN_GENERATION_NUMBER + 1);
  private static final CacheStatusResponse TEST_LOCATIONS_LOCAL = new CacheStatusResponse(Lists.newArrayList(new BlockLocation(Location.LOCAL, TEST_REMOTE_LOCATION)), CacheUtil.UNKONWN_GENERATION_NUMBER + 1);
  ...
  public void testValidateCachingBehavior_wrongInitialCacheStatus() throws TException
  {
    ...
    ReadResponse response = new ReadResponse(true, CacheUtil.UNKONWN_GENERATION_NUMBER);
  ...
  {
    final BookKeeper bookKeeper = mock(BookKeeper.class);
    ReadResponse response = new ReadResponse(false, CacheUtil.UNKONWN_GENERATION_NUMBER);
    when(bookKeeper.readData(anyString(), anyLong(), anyInt(), anyLong(), anyLong(), anyInt())).thenReturn(response);
