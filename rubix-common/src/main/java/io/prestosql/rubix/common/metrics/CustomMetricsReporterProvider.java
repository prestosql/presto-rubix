 ...
 */
package io.prestosql.rubix.common.metrics;

import com.codahale.metrics.MetricRegistry;
import io.prestosql.rubix.spi.CacheConfig;
import io.prestosql.rubix.common.utils.ClusterUtil;
    ...
    if (customMetricsReporterProvider == null) {
      synchronized (CustomMetricsReporterProvider.class) {
        if (customMetricsReporterProvider == null) {
          ...
          boolean useCustomReporter = ClusterUtil.getMetricsReporters(configuration).contains(MetricsReporterType.CUSTOM);
          CustomMetricsReporter customMetricsReporter;
          if (useCustomReporter && !className.equals("io.prestosql.rubix.common.metrics.NoOpReporter")) {
            try {
