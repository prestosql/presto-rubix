...
import io.prestosql.rubix.common.metrics.CustomMetricsReporterProvider;
...
import io.prestosql.rubix.common.metrics.CachingFileSystemMetrics;
...
public class NonLocalReadRequestChain extends ReadRequestChain
{
  ...
  {
    super(CacheUtil.UNKONWN_GENERATION_NUMBER);
      ...
      throws Exception
  {
    ...
    for (ReadRequest readRequest : readRequests) {
      ...
      try (DataTransferClientFactory.DataTransferClient dataTransferClient = DataTransferClientFactory.getClient(remoteNodeName, conf)) {
        int nread = 0;
        /*
    ...
    if (CacheConfig.isDummyModeEnabled(conf)) {
      try (RetryingPooledBookkeeperClient bookKeeperClient = bookKeeperFactory.createBookKeeperClient(remoteNodeName, conf)) {
        for (ReadRequest readRequest : readRequests) {
          ...
          bookKeeperClient.setAllCached(remotePath, fileSize, lastModified, startBlock, endBlock, CacheUtil.DUMMY_MODE_GENERATION_NUMBER);
        }
      }
      catch (Exception e) {
        if (strictMode) {
