 ...
 */
package io.prestosql.rubix.core;

import io.prestosql.rubix.common.utils.DataGen;
...
public class TestCachedReadRequestChain
{
  ...
  public void setup() throws IOException
  {
    ...
    String cachedLocalFile = CacheUtil.getLocalPath(backendFilePath.toString(), conf, CacheUtil.UNKONWN_GENERATION_NUMBER + 1);
  ...
  {
    File localFile = new File(CacheUtil.getLocalPath(backendFilePath.toString(), conf, CacheUtil.UNKONWN_GENERATION_NUMBER + 1));
    localFile.delete();
  ...
  public void testCachedRead_WithFileCached() throws IOException
  {
    ...
    assertTrue(stats.getCachedRRCDataRead() == readSize, "All data should be read from cache");
    assertTrue(stats.getDirectRRCDataRead() == 0, "No data should be read from object store");
  }
  ...
  {
    byte[] buffer = new byte[1000];
    String localCachedFile = CacheUtil.getLocalPath(backendFilePath.toString(), conf, CacheUtil.UNKONWN_GENERATION_NUMBER + 1);
    ...
    String expectedOutput = DataGen.getExpectedOutput(readSize);
    assertTrue(expectedOutput.equals(output), "Wrong data read, expected\n" + expectedOutput + "\nBut got\n" + output);
    assertTrue(stats.getCachedRRCDataRead() == 0, "No data should be read from cache");
    assertTrue(stats.getDirectRRCDataRead() == readSize, "All data should be read from object store");
  }
  ...
  public void testCachedRead_WithCorruptedLocalCachedFile_1() throws IOException
  {
    ...
    assertTrue(expectedOutput.equals(output), "Wrong data read, expected\n" + expectedOutput + "\nBut got\n" + output);
    assertTrue(stats.getCachedRRCDataRead() == 0, "No data should be read from cache");
    assertTrue(stats.getDirectRRCDataRead() == 1000, "Data read from object store didn't match");
  }
  ...
  private CachedReadRequestChain getCachedReadRequestChain(byte[] buffer) throws IOException
  {
    ...
    CachedReadRequestChain cachedReadRequestChain = new CachedReadRequestChain(fs.getRemoteFileSystem(), backendFilePath.toString(), conf, factory, CacheUtil.UNKONWN_GENERATION_NUMBER + 1);
